...
connected wilderness maps plus six scenery-only chunks (21 placed total). Their chunk coordinates were derived from the game's own
...
| `NORTH_BASIN_REGIONAL_CHUNK_DEFINITIONS` | `content/maps/north-basin-maps.js` | the 10 `NORTH_BASIN_*_MAP` (incl. five scenery-only water/woods chunks) |
...
placed regional map** (21 total — 15 accessible wilderness chunks + six scenery-only chunks: the West Outfall and five North Basin water/woods chunks). An authored definition OWNS:
...
**Regional chunks vs discrete maps.** Only the 21 placed regional chunks (15 accessible wilderness chunks + six scenery-only chunks: the West Outfall and five North Basin water/woods chunks) are
...
- **Physical map id → outdoor content-location key: ONE declarative authority.**
  ...
  (data.js)** is the single declarative binding of each of the 21 region-placed
  ...
  decorations unaffected). It covers ONLY the 21 placed outdoor maps — never
