...
`displayName`. It holds one region, `'overworld'`, containing the 19 accessible
wilderness maps plus seven scenery-only chunks (26 placed total). Their chunk coordinates were derived from the game's own
...
placed regional map** (26 total — 19 accessible wilderness chunks + seven scenery-only chunks). An authored definition OWNS:
...
**Regional chunks vs discrete maps.** Only the 26 placed regional chunks (19 accessible wilderness chunks + seven scenery-only chunks) are
...
- **`INTENTIONAL_DISCRETE` vs `NEEDS_REMAP`.** The four directed edges of
  ...
  **`INTENTIONAL_DISCRETE`** — a point crossing to the correct placed neighbour whose
  ...
  untouched. Current totals: ALIGNS 40 / NEEDS_REMAP 0 / INTENTIONAL_DISCRETE 4
  / BLOCKED 36 / BORDER 24. `validateRegionalPresentation()` checks recognized values on
...
contains **46 directed segment entries / 23 reciprocal segment pairs** across 40
...
- **Closure.** With this conversion the regional audit reaches **zero `NEEDS_REMAP`**:
  all 104 directed overworld edges classify as `ALIGNS` (40), `INTENTIONAL_DISCRETE` (4),
  `BLOCKED` (36), or `BORDER` (24); every `ALIGNS` edge is backed by the fail-closed
  `continuousSeamEntries()` authority (46 directed segment entries because split
  ...
  INTENTIONAL_DISCRETE home crossings reconnect into one traversable 19-map graph: the
...
- **Physical map id → outdoor content-location key: ONE declarative authority.**
  ...
  (data.js)** is the single declarative binding of each of the 26 region-placed
  ...
  decorations unaffected). It covers ONLY the 26 placed outdoor maps — never
...
As of the last full pass: **0 errors, 4 warnings** across 113 maps, 27,120
