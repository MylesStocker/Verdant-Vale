...
connected wilderness maps plus five scenery-only chunks (20 placed total). Their chunk coordinates were derived from the game's own
...
| `NORTH_BASIN_REGIONAL_CHUNK_DEFINITIONS` | `content/maps/north-basin-maps.js` | the 9 `NORTH_BASIN_*_MAP` (incl. four scenery-only water/woods chunks) |
...
placed regional map** (20 total — 15 accessible wilderness chunks + five scenery-only chunks: the West Outfall and four North Basin water/woods chunks). An authored definition OWNS:
...
- The chunk still needs a valid `encounterProfileId` (the catalog requires a resolved pool
  ...
  five scenery-only chunks: **`DRENWICK_WEST_OUTFALL_MAP`** (the West Outfall, chunk (1,3) — a
  drought-exposed canal settling ground whose surface water enters a buried culvert beneath
  the western ridge) and four North Basin water/woods chunks — **`NORTH_BASIN_N_MAP`** (2,0),
...
**Regional chunks vs discrete maps.** Only the 20 placed regional chunks (15 accessible wilderness chunks + five scenery-only chunks: the West Outfall and four North Basin water/woods chunks) are
...
- **Physical map id → outdoor content-location key: ONE declarative authority.**
  ...
  (data.js)** is the single declarative binding of each of the 20 region-placed
  ...
  decorations unaffected). It covers ONLY the 20 placed outdoor maps — never
