...
module.exports = {
  ...
  run() {
    ...
    assert.deepEqual(audit.seamReadiness.totals, { INTENTIONAL_DISCRETE: 4, BORDER: 26, ALIGNS: 32, BLOCKED: 30 },
      'audit totals: 92 edges -> ALIGNS 32 / BORDER 26 / BLOCKED 30 / INTENTIONAL_DISCRETE 4');
    assert.equal(audit.seamReadiness.edges.length, 92, '92 directed placed-map edges (23 x 4)');
    ...
    assert.equal(placed, 23, '23 placed chunks in the 5x6 envelope');
    assert.equal(voids, 7, '7 remaining sparse void cells');
