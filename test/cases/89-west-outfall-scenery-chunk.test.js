...
module.exports = {
  ...
  run() {
    ...
    assert.deepEqual(audit.seamReadiness.totals, { INTENTIONAL_DISCRETE: 4, BORDER: 24, ALIGNS: 26, BLOCKED: 22 },
      'audit totals: 76 edges -> ALIGNS 26 / BORDER 24 / BLOCKED 22 / INTENTIONAL_DISCRETE 4');
    assert.equal(audit.seamReadiness.edges.length, 76, '76 directed placed-map edges (19 x 4)');
    ...
    assert.equal(placed, 19, '19 placed chunks in the 5x6 envelope');
    assert.equal(voids, 11, '11 remaining sparse void cells');
