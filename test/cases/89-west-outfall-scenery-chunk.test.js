...
module.exports = {
  ...
  run() {
    ...
    assert.equal(voids, 0, 'no sparse void cells remain');
    ...
    // ── 20. The former final sparse coordinate now resolves Mirewood ──────────
    assert.equal(g.run("mapIdForChunk('overworld', 0, 2)"), 'NORTH_BASIN_MIREWOOD_MAP', '(0,2) is occupied by Mirewood');
    assert.equal(g.run("tileAtWorld('overworld', 8, 2*ROWS+7)"), g.run("mapRefForId('NORTH_BASIN_MIREWOOD_MAP')[7][8]"), '(0,2) reads Mirewood terrain');
    assert.equal(g.run("worldToLocal('overworld', 8, 2*ROWS+7).mapId"), 'NORTH_BASIN_MIREWOOD_MAP', '(0,2) worldToLocal resolves Mirewood');
