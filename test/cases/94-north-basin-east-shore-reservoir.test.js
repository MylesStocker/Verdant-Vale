...
module.exports = {
  ...
  run() {
    ...
    assert.equal(GRID_FP.fingerprints[ID], FP, 'fixture records it'); assert.equal(Object.keys(GRID_FP.fingerprints).length, 26, 'all regional grids fingerprinted');
    const audit = require('../transition-audit.js'), v = Object.fromEntries(audit.seamReadiness.edges.map((e) => [e.mapId + '|' + e.dir, e.verdict]));
    assert.equal(v[`${ID}|north`], 'BLOCKED'); assert.equal(v[`${ID}|west`], 'BLOCKED'); assert.equal(v[`${ID}|east`], 'BORDER'); assert.equal(v[`${ID}|south`], 'BORDER');
    assert.equal(v['NORTH_BASIN_NE2_MAP|south'], 'BLOCKED'); assert.equal(v['NORTH_BASIN_E_MAP|east'], 'BLOCKED');
