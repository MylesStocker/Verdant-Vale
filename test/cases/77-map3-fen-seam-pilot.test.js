...
module.exports = {
  ...
  run() {
    ...
    assert.equal(audit.seamReadiness.totals.ALIGNS, 22, 'ALIGNS 22 (incl. the MAP3<->MAP3_N1 pilot, MAP2<->MAP3, MAP3<->MAP4, and MAP4<->MAP5 seams)');
    ...
    assert.equal(g.run('continuousSeamEntries().length'), 22, '22 eligible directed seams');
    ...
    assert.equal(V['MAP_N1|north'], 'NEEDS_REMAP', 'MAP_N1|north still NEEDS_REMAP (northern point crossing untouched)');
