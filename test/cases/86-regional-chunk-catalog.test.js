...
const EXPECTED = ['MAP', 'MAP2', 'MAP3', 'MAP4', 'MAP5', 'MAP_N1', 'MAP_N2', 'RODDON_WAY_MAP',
  ...
  'NORTH_BASIN_SW_MAP', 'NORTH_BASIN_MIREWOOD_MAP', 'NORTH_BASIN_W_MAP', 'NORTH_BASIN_NW2_MAP', 'NORTH_BASIN_W2_MAP', 'NORTH_BASIN_NW_MAP', 'NORTH_BASIN_N_MAP',
...
// for legacy consumers. Newer chunks, including every scenery-only addition, were
...
const ALIASLESS = ['DRENWICK_EAST_CANAL_MAP', 'THORNMERE_NORTH_FEN_MAP', 'THORNMERE_CANAL_HEAD_MAP', 'THORNMERE_UPPER_SHALLOWS_MAP', 'DRENWICK_WEST_OUTFALL_MAP', 'NORTH_BASIN_N_MAP', 'NORTH_BASIN_NE_MAP', 'NORTH_BASIN_NE2_MAP', 'NORTH_BASIN_E2_MAP', 'NORTH_BASIN_E_MAP', 'NORTH_BASIN_SE_MAP', 'EAST_CAUSEWAY_MAP', 'NORTH_BASIN_NW2_MAP', 'NORTH_BASIN_W2_MAP', 'NORTH_BASIN_MIREWOOD_MAP'];
...
const FRAGMENTS = {
  ...
  NORTH_BASIN_REGIONAL_CHUNK_DEFINITIONS: ['NORTH_BASIN_S_MAP', 'NORTH_BASIN_SE_MAP', 'EAST_CAUSEWAY_MAP', 'NORTH_BASIN_C_MAP',
    'NORTH_BASIN_MIREWOOD_MAP', 'NORTH_BASIN_SW_MAP', 'NORTH_BASIN_W_MAP', 'NORTH_BASIN_NW2_MAP', 'NORTH_BASIN_W2_MAP', 'NORTH_BASIN_NW_MAP', 'NORTH_BASIN_N_MAP', 'NORTH_BASIN_NE_MAP', 'NORTH_BASIN_NE2_MAP', 'NORTH_BASIN_E2_MAP', 'NORTH_BASIN_E_MAP'],
};

module.exports = {
  name: 'Regional chunk authoring: distributed fragments → resolved catalog, all 30 grids inline',
  run() {
    const g = createContext();
    g.press('Enter'); g.press('Enter');
    const J = (e) => JSON.parse(g.run(e));

    // ── 1. Authored fragments collectively contain all 30 maps exactly once ───
    ...
    assert.deepEqual([...seenAcross.keys()].sort(), [...EXPECTED].sort(), 'the fragments collectively author exactly the 30 placed maps');
    // merged definition list = the union, deduped, and coordinates unique
    assert.equal(g.run('_REGIONAL_CHUNK_DEFINITIONS.length'), 30, 'the merged _REGIONAL_CHUNK_DEFINITIONS has 30 definitions');
    const defCoords = J("JSON.stringify(_REGIONAL_CHUNK_DEFINITIONS.map(function(d){return d.regionId+':'+d.chunkX+','+d.chunkY;}))");
    assert.equal(new Set(defCoords).size, 30, 'no chunk coordinate is authored in more than one fragment');

    // ── 3. The resolved runtime catalog has exactly the same 30 entries ───────
    const ids = J('JSON.stringify(Object.keys(REGIONAL_CHUNK_CATALOG))');
    assert.equal(ids.length, 30, 'exactly 30 resolved chunk records');
    ...
    assert.equal(g.run('REGIONAL_LAYOUT.overworld.placements.length'), 30, 'REGIONAL_LAYOUT has 30 placements');
    assert.equal(g.run('Object.keys(OUTDOOR_CONTENT_KEYS).length'), 30, 'OUTDOOR_CONTENT_KEYS has 30 entries');
    ...
    // ── 9-of-spec (1-4,7). Completed migration: ALL 26 grids authored inline in
    ...
    const SRC = {
      ...
      THORNMERE_NORTH_FEN_MAP: 'content/maps/thornmere-wilds-maps.js',
      THORNMERE_CANAL_HEAD_MAP: 'content/maps/thornmere-wilds-maps.js',
      THORNMERE_UPPER_SHALLOWS_MAP: 'content/maps/thornmere-wilds-maps.js',
      ...
      NORTH_BASIN_SW_MAP: 'content/maps/north-basin-maps.js', NORTH_BASIN_MIREWOOD_MAP: 'content/maps/north-basin-maps.js', NORTH_BASIN_W_MAP: 'content/maps/north-basin-maps.js',
      NORTH_BASIN_NW2_MAP: 'content/maps/north-basin-maps.js',
      NORTH_BASIN_W2_MAP: 'content/maps/north-basin-maps.js',
    ...
    assert.deepEqual(Object.keys(GRID_FP.fingerprints).sort(), [...EXPECTED].sort(), 'fixture fingerprints exactly the 30 placed regional grids');
    for (const id of EXPECTED) {
