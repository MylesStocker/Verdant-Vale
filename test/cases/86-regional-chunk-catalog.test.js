...
const EXPECTED = ['MAP', 'MAP2', 'MAP3', 'MAP4', 'MAP5', 'MAP_N1', 'MAP_N2', 'RODDON_WAY_MAP',
  ...
  'NORTH_BASIN_SW_MAP', 'NORTH_BASIN_W_MAP', 'NORTH_BASIN_NW2_MAP', 'NORTH_BASIN_W2_MAP', 'NORTH_BASIN_NW_MAP', 'NORTH_BASIN_N_MAP',
...
const FRAGMENTS = {
  ...
  NORTH_BASIN_REGIONAL_CHUNK_DEFINITIONS: ['NORTH_BASIN_S_MAP', 'NORTH_BASIN_SE_MAP', 'EAST_CAUSEWAY_MAP', 'NORTH_BASIN_C_MAP',
    'NORTH_BASIN_SW_MAP', 'NORTH_BASIN_W_MAP', 'NORTH_BASIN_NW2_MAP', 'NORTH_BASIN_W2_MAP', 'NORTH_BASIN_NW_MAP', 'NORTH_BASIN_N_MAP', 'NORTH_BASIN_NE_MAP', 'NORTH_BASIN_NE2_MAP', 'NORTH_BASIN_E2_MAP', 'NORTH_BASIN_E_MAP'],
};

module.exports = {
  name: 'Regional chunk authoring: distributed fragments → resolved catalog, all 29 grids inline',
  run() {
    const g = createContext();
    g.press('Enter'); g.press('Enter');
    const J = (e) => JSON.parse(g.run(e));

    // ── 1. Authored fragments collectively contain all 29 maps exactly once ───
    ...
    assert.deepEqual([...seenAcross.keys()].sort(), [...EXPECTED].sort(), 'the fragments collectively author exactly the 29 placed maps');
    // merged definition list = the union, deduped, and coordinates unique
    assert.equal(g.run('_REGIONAL_CHUNK_DEFINITIONS.length'), 29, 'the merged _REGIONAL_CHUNK_DEFINITIONS has 29 definitions');
    const defCoords = J("JSON.stringify(_REGIONAL_CHUNK_DEFINITIONS.map(function(d){return d.regionId+':'+d.chunkX+','+d.chunkY;}))");
    assert.equal(new Set(defCoords).size, 29, 'no chunk coordinate is authored in more than one fragment');

    // ── 3. The resolved runtime catalog has exactly the same 29 entries ───────
    const ids = J('JSON.stringify(Object.keys(REGIONAL_CHUNK_CATALOG))');
    assert.equal(ids.length, 29, 'exactly 29 resolved chunk records');
    ...
    assert.equal(g.run('REGIONAL_LAYOUT.overworld.placements.length'), 29, 'REGIONAL_LAYOUT has 29 placements');
    assert.equal(g.run('Object.keys(OUTDOOR_CONTENT_KEYS).length'), 29, 'OUTDOOR_CONTENT_KEYS has 29 entries');
    ...
    assert.deepEqual(Object.keys(GRID_FP.fingerprints).sort(), [...EXPECTED].sort(), 'fixture fingerprints exactly the 29 placed regional grids');
    for (const id of EXPECTED) {
    ...
    assert.equal(g.run('continuousSeamEntries().length'), 64, '64 eligible directed segment entries');
    for (const id of EXPECTED) {
