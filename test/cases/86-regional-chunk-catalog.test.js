...
const EXPECTED = ['MAP', 'MAP2', 'MAP3', 'MAP4', 'MAP5', 'MAP_N1', 'MAP_N2', 'RODDON_WAY_MAP',
  ...
  'NORTH_BASIN_SW_MAP', 'NORTH_BASIN_W_MAP', 'NORTH_BASIN_W2_MAP', 'NORTH_BASIN_NW_MAP', 'NORTH_BASIN_N_MAP',
...
const ALIASLESS = ['DRENWICK_EAST_CANAL_MAP', 'THORNMERE_NORTH_FEN_MAP', 'THORNMERE_CANAL_HEAD_MAP', 'THORNMERE_UPPER_SHALLOWS_MAP', 'DRENWICK_WEST_OUTFALL_MAP', 'NORTH_BASIN_N_MAP', 'NORTH_BASIN_NE_MAP', 'NORTH_BASIN_NE2_MAP', 'NORTH_BASIN_E2_MAP', 'NORTH_BASIN_E_MAP', 'NORTH_BASIN_SE_MAP', 'EAST_CAUSEWAY_MAP', 'NORTH_BASIN_W2_MAP'];
...
const FRAGMENTS = {
  ...
  NORTH_BASIN_REGIONAL_CHUNK_DEFINITIONS: ['NORTH_BASIN_S_MAP', 'NORTH_BASIN_SE_MAP', 'EAST_CAUSEWAY_MAP', 'NORTH_BASIN_C_MAP',
    'NORTH_BASIN_SW_MAP', 'NORTH_BASIN_W_MAP', 'NORTH_BASIN_W2_MAP', 'NORTH_BASIN_NW_MAP', 'NORTH_BASIN_N_MAP', 'NORTH_BASIN_NE_MAP', 'NORTH_BASIN_NE2_MAP', 'NORTH_BASIN_E2_MAP', 'NORTH_BASIN_E_MAP'],
};

module.exports = {
  name: 'Regional chunk authoring: distributed fragments → resolved catalog, all 28 grids inline',
  run() {
    const g = createContext();
    g.press('Enter'); g.press('Enter');
    const J = (e) => JSON.parse(g.run(e));

    // ── 1. Authored fragments collectively contain all 28 maps exactly once ───
    ...
    assert.deepEqual([...seenAcross.keys()].sort(), [...EXPECTED].sort(), 'the fragments collectively author exactly the 28 placed maps');
    // merged definition list = the union, deduped, and coordinates unique
    assert.equal(g.run('_REGIONAL_CHUNK_DEFINITIONS.length'), 28, 'the merged _REGIONAL_CHUNK_DEFINITIONS has 28 definitions');
    const defCoords = J("JSON.stringify(_REGIONAL_CHUNK_DEFINITIONS.map(function(d){return d.regionId+':'+d.chunkX+','+d.chunkY;}))");
    assert.equal(new Set(defCoords).size, 28, 'no chunk coordinate is authored in more than one fragment');

    // ── 3. The resolved runtime catalog has exactly the same 28 entries ───────
    const ids = J('JSON.stringify(Object.keys(REGIONAL_CHUNK_CATALOG))');
    assert.equal(ids.length, 28, 'exactly 28 resolved chunk records');
    ...
    assert.equal(g.run('REGIONAL_LAYOUT.overworld.placements.length'), 28, 'REGIONAL_LAYOUT has 28 placements');
    assert.equal(g.run('Object.keys(OUTDOOR_CONTENT_KEYS).length'), 28, 'OUTDOOR_CONTENT_KEYS has 28 entries');
    ...
    assert.deepEqual(Object.keys(GRID_FP.fingerprints).sort(), [...EXPECTED].sort(), 'fixture fingerprints exactly the 28 placed regional grids');
    for (const id of EXPECTED) {
