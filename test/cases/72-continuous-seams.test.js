...
module.exports = {
  ...
  run() {
    ...
    assert.equal(entries.length, 56, '56 eligible directed segment entries (28 reciprocal segment pairs)');
    ...
    g.run(`EDGE_TRANSITIONS.NORTH_BASIN_S_MAP.north=window.__multiOldA;
      ...
      _CS_INDEX=null;_CS_MAPS=null;`);
    assert.equal(g.run('continuousSeamEntries().length'), 56, 'all production seam segments restore unchanged');
  },
};
