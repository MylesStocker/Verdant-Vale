...
module.exports = {
  ...
  run() {
    ...
    assert.equal(g.run('continuousSeamEntries().length'),68); assert.equal(g.run('REGIONAL_LAYOUT.overworld.placements.length'),30); assert.equal(g.run('Object.keys(MAP_CATALOG).length'),126); assert.equal(g.run('SAVE_VERSION'),4);
  },
};
