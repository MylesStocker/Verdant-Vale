...
module.exports = {
  ...
  run() {
    ...
    const placements = JSON.parse(g.run(
      `JSON.stringify(REGIONAL_LAYOUT['${regionId}'].placements)`));
    ...
    for (const p of placements) {
      ...
      assert.deepEqual(back, { regionId, mapId: p.mapId, chunkX: p.chunkX, chunkY: p.chunkY },
        `regionPlacementForMapId round-trips ${p.mapId}`);
      // The layout ID is exposed as `regionId`, never as a legacy `region` property.
      assert.equal(back.regionId, regionId, `${p.mapId} placement exposes regionId`);
      assert.ok(!('region' in back), `${p.mapId} placement must NOT expose a legacy 'region' property`);
      assert.equal(g.run(`mapIdForChunk('${regionId}', ${p.chunkX}, ${p.chunkY})`), p.mapId,
    ...
    for (const p of placements) {
      for (const [lx, ly] of corners) {
        ...
        assert.deepEqual(w, { regionId, worldX: p.chunkX * COLS + lx, worldY: p.chunkY * ROWS + ly },
          `localToWorld(${p.mapId},${lx},${ly})`);
        assert.ok(!('region' in w), `localToWorld must NOT expose a legacy 'region' property`);
        const back = JSON.parse(g.run(`JSON.stringify(worldToLocal('${regionId}', ${w.worldX}, ${w.worldY}))`));
    ...
    assert.equal(g.run(`worldToLocal('${regionId}', 15, 75).mapId`), 'MAP', 'worldX 15 is MAP east edge');
    assert.equal(g.run(`worldToLocal('${regionId}', 15, 75).localX`), 15, 'MAP east edge is local col 15');
    assert.equal(g.run(`worldToLocal('${regionId}', 16, 75).mapId`), 'MAP2', 'worldX 16 crosses into MAP2');
    assert.equal(g.run(`worldToLocal('${regionId}', 16, 75).localX`), 0, 'MAP2 west edge is local col 0');

    // ── 5. tileAtWorld matches a direct tile read (a real placed tile) ──────
    const sample = JSON.parse(g.run(`(()=>{
      var w = localToWorld('MAP', 8, 8);
      return JSON.stringify({ world: tileAtWorld('${regionId}', w.worldX, w.worldY), direct: mapRefForId('MAP')[8][8] });
    })()`));
    ...
    // negative
    assert.equal(g.run(`worldToLocal('${regionId}', -1, 0)`), null, 'negative worldX -> null');
    assert.equal(g.run(`tileAtWorld('${regionId}', -1, 0)`), VOID, 'negative worldX -> void');
    ...
    assert.equal(seam.regionId, regionId, 'seam report covers the overworld region');
