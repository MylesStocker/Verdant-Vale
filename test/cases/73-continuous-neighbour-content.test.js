...
module.exports = {
  ...
  run() {
    ...
    assert.equal(ents.length, 24, 'all 24 placed outdoor maps are bound');
    ...
    const expected = { MAP:'overworld', MAP2:'map2', MAP3:'map3', MAP4:'map4', MAP5:'overworld',
      ...
      DRENWICK_EAST_CANAL_MAP:'drenwick_east_canal', THORNMERE_NORTH_FEN_MAP:'thornmere_north_fen',
    ...
    assert.equal(ents.filter(e => e.unambiguous).length, 21, 'the other 21 bindings are unique');
