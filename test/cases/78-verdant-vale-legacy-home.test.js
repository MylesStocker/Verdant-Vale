...
module.exports = {
  ...
  run() {
    ...
    assert.deepEqual(audit.seamReadiness.totals, { INTENTIONAL_DISCRETE: 4, BORDER: 26, ALIGNS: 22, NEEDS_REMAP: 4, BLOCKED: 4 }, 'totals: ALIGNS 22 / NEEDS_REMAP 4 / INTENTIONAL_DISCRETE 4 / BLOCKED 4 / BORDER 26');
    ...
    assert.deepEqual([...nr].sort(), ['MAP3_N2<->NORTH_BASIN_S_MAP', 'MAP_N1<->MAP_N2'].sort(), 'the 2 remaining NEEDS_REMAP pairs are exactly as expected');
    assert.equal(g.run('continuousSeamEntries().length'), 22, '22 eligible directed seams (11 pairs)');
    ...
    assert.ok(g.run("typeof REGIONAL_POINT_CROSSINGS !== 'undefined' && REGIONAL_POINT_CROSSINGS.length === 8"), 'REGIONAL_POINT_CROSSINGS is the shared production authority (8 directed regional crossings)');
    ...
    assert.equal(g.run("REGIONAL_POINT_CROSSINGS.length"), 8, 'REGIONAL_POINT_CROSSINGS intact after all negative fixtures');
