...
module.exports = {
  ...
  run() {
    ...
    assert.deepEqual(audit.seamReadiness.totals, { INTENTIONAL_DISCRETE: 4, BORDER: 26, ALIGNS: 16, NEEDS_REMAP: 10, BLOCKED: 4 }, 'totals: ALIGNS 16 / NEEDS_REMAP 10 / INTENTIONAL_DISCRETE 4 / BLOCKED 4 / BORDER 26');
    ...
    assert.deepEqual([...nr].sort(), ['MAP2<->MAP3', 'MAP3<->MAP4', 'MAP4<->MAP5', 'MAP3_N2<->NORTH_BASIN_S_MAP', 'MAP_N1<->MAP_N2'].sort(), 'the 5 remaining NEEDS_REMAP pairs are exactly as expected');
    assert.equal(g.run('continuousSeamEntries().length'), 16, 'still 16 eligible directed seams (8 pairs)');
    ...
    assert.ok(g.run("typeof REGIONAL_POINT_CROSSINGS !== 'undefined' && REGIONAL_POINT_CROSSINGS.length === 14"), 'REGIONAL_POINT_CROSSINGS is the shared production authority (14 directed regional crossings)');
    ...
    assert.equal(g.run("REGIONAL_POINT_CROSSINGS.length"), 14, 'REGIONAL_POINT_CROSSINGS intact after all negative fixtures');
