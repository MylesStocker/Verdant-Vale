...
module.exports = {
  ...
  run() {
    ...
    assert.deepEqual(audit.seamReadiness.totals, { INTENTIONAL_DISCRETE: 4, BORDER: 26, ALIGNS: 24, NEEDS_REMAP: 2, BLOCKED: 4 }, 'totals: ALIGNS 24 / NEEDS_REMAP 2 / INTENTIONAL_DISCRETE 4 / BLOCKED 4 / BORDER 26');
    ...
    assert.deepEqual([...nr].sort(), ['MAP3_N2<->NORTH_BASIN_S_MAP'].sort(), 'the 1 remaining NEEDS_REMAP pair is exactly as expected');
    assert.equal(g.run('continuousSeamEntries().length'), 24, '24 eligible directed seams (12 pairs)');
    ...
    assert.ok(g.run("typeof REGIONAL_POINT_CROSSINGS !== 'undefined' && REGIONAL_POINT_CROSSINGS.length === 6"), 'REGIONAL_POINT_CROSSINGS is the shared production authority (6 directed regional crossings)');
    ...
    assert.equal(g.run("REGIONAL_POINT_CROSSINGS.length"), 6, 'REGIONAL_POINT_CROSSINGS intact after all negative fixtures');
