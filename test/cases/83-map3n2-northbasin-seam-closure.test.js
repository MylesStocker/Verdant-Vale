...
// Verdant Vale legacy-home crossings, and the 19-map graph traversably connected
...
module.exports = {
  name: 'MAP3_N2<->NORTH_BASIN_S_MAP seam + regional closure: zero NEEDS_REMAP, connected 19-map graph',
  run() {
    const g = ctx();
    ...
    assert.equal(edges.length, 104, '104 directed regional edges total (26 placed maps x 4 sides)');
    ...
    assert.deepEqual(audit.seamReadiness.totals, { INTENTIONAL_DISCRETE: 4, BORDER: 24, ALIGNS: 40, BLOCKED: 36 }, 'closure totals: ALIGNS 40 / INTENTIONAL_DISCRETE 4 / BLOCKED 36 / BORDER 24');
    ...
    assert.equal(alignsEdges.length, seamSet.size, 'the ALIGNS physical-edge set equals the eligible continuous-edge set exactly (32; segment entries are counted separately)');
    ...
    //        adding the intentional-discrete home crossings reconnects all 17 ACCESSIBLE
    ...
    assert.equal(allNodes.length, 26, '26 placed regional maps (19 accessible + 7 scenery-only)');
    const nodes = allNodes.filter((n) => g.run('mapPlayerAccessible(' + JSON.stringify(n) + ')'));
    assert.equal(nodes.length, 19, '19 accessible regional maps form the traversable world');
    ...
    assert.equal(components(nodes, contPairs.concat(homePairs)), 1, 'adding the intentional-discrete legacy-home crossings connects all 19 accessible maps into one traversable graph');
