...
// Verdant Vale legacy-home crossings, and the 16-map graph traversably connected
...
module.exports = {
  name: 'MAP3_N2<->NORTH_BASIN_S_MAP seam + regional closure: zero NEEDS_REMAP, connected 16-map graph',
  run() {
    const g = ctx();
    ...
    assert.equal(edges.length, 88, '88 directed regional edges total (22 placed maps x 4 sides)');
    ...
    assert.deepEqual(audit.seamReadiness.totals, { INTENTIONAL_DISCRETE: 4, BORDER: 26, ALIGNS: 28, BLOCKED: 30 }, 'closure totals: ALIGNS 28 / INTENTIONAL_DISCRETE 4 / BLOCKED 30 / BORDER 26');
    ...
    assert.equal(alignsEdges.length, seamSet.size, 'the ALIGNS set equals the eligible continuous-seam set exactly (28)');
    ...
    //        adding the intentional-discrete home crossings reconnects all 16 ACCESSIBLE
    ...
    assert.equal(allNodes.length, 22, '22 placed regional maps (16 accessible + 6 scenery-only)');
    const nodes = allNodes.filter((n) => g.run('mapPlayerAccessible(' + JSON.stringify(n) + ')'));
    assert.equal(nodes.length, 16, '16 accessible regional maps form the traversable world');
    ...
    assert.equal(components(nodes, contPairs.concat(homePairs)), 1, 'adding the intentional-discrete legacy-home crossings connects all 16 accessible maps into one traversable graph');
